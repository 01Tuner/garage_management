...
class ServiceJob(Document):
	...
	def before_submit(self):
		if self.status == "Cancelled":
			frappe.throw(_("Cannot submit a Cancelled Service Job"))
	...
	def send_status_email_if_needed(self):
		...
		if settings.notification_recipients:
			...
			email = frappe.db.get_value("User", self.technician, "email")
			if email:
				recipients = [email]
		...
		try:
			frappe.sendmail(
				...
				subject=_("Service Job {0}: {1}").format(self.name, self.status),
				message=_(
					"<p>Service Job <b>{0}</b> for <b>{1}</b> is now <b>{2}</b>.</p>"
					"<p>Complaint: {3}</p>"
				).format(
					self.name,
			...
			frappe.log_error(title="Garage Service Job status email failed")
...
def get_permission_query_conditions(user=None):
	...
	if "Garage Technician" in frappe.get_roles(user) and not any(
		r in frappe.get_roles(user) for r in ("Garage Manager", "Garage Front Desk", "Accounts User")
	):
		return f"`tabService Job`.technician = {frappe.db.escape(user)}"
...
def has_permission(doc, ptype=None, user=None):
	...
	if "Garage Technician" in frappe.get_roles(user) and not any(
		r in frappe.get_roles(user) for r in ("Garage Manager", "Garage Front Desk")
	):
